interface FlipCardProps {
  cardNumber: string
  cardName:   string
  cardExpiry: string
  cardCvc:    string
}

export function FlipCard({ cardNumber, cardName, cardExpiry, cardCvc }: FlipCardProps) {
  const displayNumber = cardNumber
    ? cardNumber.padEnd(19, ' ').replace(/ {1,4}/g, m => m === '    ' ? ' •••• ' : m).trim()
    : '•••• •••• •••• ••••'

  const displayName   = cardName.trim()   || 'YOUR NAME'
  const displayExpiry = cardExpiry.trim() || 'MM/YY'
  const displayCvc    = cardCvc.trim()    || '•••'

  return (
    <div className="flip-card">
      <div className="flip-card-inner">

        {/* ── Front ── */}
        <div className="flip-card-front">
          <p className="fc-heading">MASTERCARD</p>

          {/* Mastercard logo */}
          <svg className="fc-logo" xmlns="http://www.w3.org/2000/svg" width={36} height={36} viewBox="0 0 48 48">
            <path fill="#ff9800" d="M32 10A14 14 0 1 0 32 38A14 14 0 1 0 32 10Z"/>
            <path fill="#d50000" d="M16 10A14 14 0 1 0 16 38A14 14 0 1 0 16 10Z"/>
            <path fill="#ff3d00" d="M18,24c0,4.755,2.376,8.95,6,11.48c3.624-2.53,6-6.725,6-11.48s-2.376-8.95-6-11.48C20.376,15.05,18,19.245,18,24z"/>
          </svg>

          {/* Chip */}
          <svg className="fc-chip" xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 50 50">
            <image width={50} height={50} x={0} y={0} href="data:image/png;base64,  " />
          </svg>

          {/* Contactless */}
          <svg className="fc-contactless" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 50 50">
            <image width={50} height={50} x={0} y={0} href="data:image/png;base64," />
          </svg>

          <p className="fc-number">{displayNumber}</p>
          <p className="fc-valid-label">VALID THRU</p>
          <p className="fc-expiry">{displayExpiry}</p>
          <p className="fc-name">{displayName}</p>
        </div>

        {/* ── Back ── */}
        <div className="flip-card-back">
          <div className="fc-strip" />
          <div className="fc-mstrip" />
          <div className="fc-sstrip">
            <p className="fc-code">{displayCvc}</p>
          </div>
        </div>

      </div>
    </div>
  )
}
